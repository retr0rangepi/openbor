...
void putsprite_ex(int x, int y, s_sprite *frame, s_screen *screen, s_drawmethod *drawmethod)
{
    ...
    if(!drawmethod->water.watermode && drawmethod->scalex == 256 && drawmethod->scaley == 256 && !drawmethod->flipping.axis.y && !drawmethod->shiftx && drawmethod->fillcolor == TRANSPARENT_IDX && !drawmethod->rotate)
    {
        ...
        {
        case PIXEL_8:
            putsprite_8(x, y, drawmethod->flipping.axis.x, frame, screen, drawmethod->table, drawmethod->alpha > 0 ? blendtables[drawmethod->alpha - 1] : NULL);
            break;
        case PIXEL_16:
            putsprite_x8p16(x, y, drawmethod->flipping.axis.x, frame, screen, (unsigned short *)drawmethod->table, getblendfunction16(drawmethod->alpha));
            break;
        case PIXEL_32:
            putsprite_x8p32(x, y, drawmethod->flipping.axis.x, frame, screen, (unsigned *)drawmethod->table, getblendfunction32(drawmethod->alpha));
            break;
        }
        return;
    }
